...
export function SocialSecurityPage() {
  ...
  >({});
  const [fetchingComparison, setFetchingComparison] = useState<
      ...
      <Box sx={{ mb: 4 }}>
        ...
        {!comp && !fetching && uploaded && (
          <Alert severity="warning" sx={{ fontSize: "0.8125rem" }}>
            Earnings are uploaded but no estimates are available. Click
            Recalculate or ensure your birth year is saved in Inputs.
          </Alert>
        )}

        {!uploaded && (
          <Alert severity="info" sx={{ fontSize: "0.8125rem" }}>
            Upload your SSA earnings history CSV to see benefit estimates. Use
            the template at{" "}
            <code>backend/database/ss_earnings_template.csv</code> — fill in
            your annual earnings from your SSA statement at
  ...
  return (
    <Box className="page-enter" sx={{ maxWidth: 900 }}>
      ...
      >
        Upload earnings history and compare claiming strategies. Benefit
        estimates persist across sessions — re-upload only if your earnings
        history changes.
      </Typography>
      <PersonSSPanel
        person={primary}
