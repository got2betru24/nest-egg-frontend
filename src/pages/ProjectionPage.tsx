...
function PortfolioGrowthChart({ years }: { years: ProjectionYear[] }) {
  ...
  const shapes: Partial<Plotly.Shape>[] = retirementYear
    ? [{
        type: 'line',
        x0: retirementYear, x1: retirementYear,
        y0: 0, y1: 1,
        yref: 'paper',
  ...
  return (
    <Plot
      data={[
        ...
        {
          x: distYears.map(y => y.calendar_year),
          y: distYears.map(y => (y.withdrawals.hysa + y.withdrawals.brokerage) / 1000),
          name: 'Taxable Accts',
          type: 'bar',
  ...
  return (
    <Plot
      data={[
        ...
        {
          x: distYears.map(y => y.calendar_year),
          y: distYears.map(y => (y.tax?.ltcg_tax ?? 0) + (y.tax?.niit ?? 0)).map(v => v / 1000),
          name: 'LTCG / NIIT',
          type: 'bar',
          marker: { color: COLORS.brokerage + 'cc' },
...
export function ProjectionPage() {
  ...
  const [chartTab, setChartTab] = useState<'portfolio' | 'pretax' | 'income' | 'tax'>('portfolio')
    ...
    <Box className="page-enter" sx={{ maxWidth: 1100 }}>
      ...
      <Box sx={{ display: 'flex', gap: 1, mb: 3, flexWrap: 'wrap' }}>
        <Chip
          ...
          size="small"
          sx={{
            bgcolor: projection.success ? 'rgba(45,212,170,0.12)' : 'rgba(248,113,113,0.12)',
            color: projection.success ? 'var(--color-positive)' : 'var(--color-negative)',
          }}
        />
        {!projection.success && (
          <Chip
      ...
      <ChartPanel title={
        ...
        : chartTab === 'income' ? 'Annual Income Sources vs Target (000s)'
