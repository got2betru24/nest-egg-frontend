    ...
    <Plot
      data={[
        ...
        },
        {
          ...
          x: distYears
            .filter((y) => y.income_gap < -100)
            .map((y) => y.calendar_year),
          y: distYears
            .filter((y) => y.income_gap < -100)
            .map((y) => y.net_income / 1000),
          name: "Shortfall",
          type: "scatter",
          mode: "markers",
          marker: { color: COLORS.spill, size: 6, symbol: "circle" },
          hovertemplate: "<b>Gap</b>: $%{customdata:.0f}K<extra></extra>",
          customdata: distYears
            .filter((y) => y.income_gap < -100)
            .map((y) => y.income_gap / 1000),
        },
      ]}
      layout={{
        ...PLOTLY_LAYOUT_BASE,
    ...
    <Plot
      data={[
        ...
        },
        {
          x: distYears.map((y) => y.calendar_year),
          y: distYears.map(
            (y) => ((y.tax?.ltcg_tax ?? 0) + (y.tax?.niit ?? 0)) / 1000
          ),
          name: "LTCG / NIIT",
          type: "bar",
          marker: { color: COLORS.brokerage + "cc" },
  ...
  const sources = [
    {
      ...
      value: y.ss_primary + y.ss_spouse,
      color: COLORS.ss,
    },
    { label: "HYSA", value: y.withdrawals.hysa, color: COLORS.hysa },
    {
      label: "Brokerage (LTCG)",
      value: y.withdrawals.brokerage,
    ...
    {
      label: "Roth",
      value: y.withdrawals.roth_ira + y.withdrawals.roth_401k,
      color: COLORS.roth_ira,
    },
    {
      label: "Roth Conversion",
      value: y.roth_ladder_conversion,
      color: "#c4b5fd",
    },
  ].filter((s) => s.value > 0);
      ...
      {open && (
        ...
        >
          <Box
            component="td"
            colSpan={9}
            sx={{ px: 2, pb: 2, pt: 1, bgcolor: "var(--bg-elevated)" }}
          >
                ...
                {y.tax && (
                  ...
                  >
                    {[
                      {
                        label: "Ordinary Tax",
                        value: formatCurrency(y.tax.tax_owed, {
                          compact: true,
                        }),
                      },
                      {
                      ...
                      {
                        label: "SS Taxable",
                        value: formatCurrency(y.tax.ss_taxable_amount, {
                          compact: true,
                        }),
                      },
                    ].map((row) => (
              ...
              <Grid2 size={{ xs: 12, md: 3 }}>
                ...
                >
                  ...
                  {y.roth_ladder_conversion > 0 && (
                    <Chip
                      label={`Roth conv. ${formatCurrency(
                        y.roth_ladder_conversion,
                        { compact: true }
                      )}`}
                      size="small"
                      sx={{
                        bgcolor: "rgba(196,181,253,0.10)",
                        color: "#c4b5fd",
                        fontSize: "0.6875rem",
                        height: 20,
                        width: "fit-content",
...
export function ProjectionPage() {
  ...
  const rmdWarning = years.some(
    (y) => y.age_primary >= 73 && y.balances_end.traditional_401k > 0
  );
    ...
    <Box className="page-enter" sx={{ maxWidth: 1100 }}>
      ...
      {rmdWarning && (
        <Alert severity="warning" sx={{ mb: 2 }}>
          <strong>RMD Notice:</strong> Traditional 401(k) balance remains past
          age 73. Required Minimum Distributions (RMDs) will apply — consult a
          tax advisor about the impact on your withdrawal plan.
        </Alert>
      )}
      ...
      <Box sx={{ mt: 3 }}>
        ...
        >
          ...
          <Typography
            component="span"
            sx={{ fontSize: "0.75rem", color: "var(--text-muted)", ml: 1 }}
          >
